...
reverseList l = if null l 
    ...
    -- reverse pe restul elementelor concatenat cu primu


-- pattern matching
reverseList2 :: [a] -> [a]
reverseList2 [] = [] -- x e primul element, xl e restul listei
reverseList2 (x : xl) = reverseList2 xl ++ [x]

-- case of
reverseList3 :: [a] -> [a]
reverseList3 l = case null l of
    True -> []
    _ -> reverseList3 (tail l) ++ [head l]
...
-- varianta2 reverseList5 l = foldl (\acc x -> acc ++ [x]) [] l

-- foldr (la foldr sunt invers x si acc)
reverseList6 :: [a] -> [a]
...
-- pattern matching (_ e wildcard, ignora tot)
